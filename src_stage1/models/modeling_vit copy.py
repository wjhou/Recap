...
class VisualEncoder(PreTrainedModel):
    def __init__(self, config):
        ...
        self.observation_det = nn.Linear(
            self.config.hidden_size * 2, config.num_observation - 1
        )
        self.observation_cls = nn.Linear(
            self.config.hidden_size * 2, config.num_observation
        )
    ...
    def forward(
        ...
        pooler_output = torch.cat((pooler_output, prior_pooler_output), dim=-1)
        observation_det_logits = self.observation_det(pooler_output)
        observation_cls_logits = self.observation_cls(pooler_output)
        progression_logits = self.progression_cls(pooler_output)
        # if require_logits:
